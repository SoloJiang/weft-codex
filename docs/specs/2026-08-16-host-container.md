...
## 5.1 详情抽屉走 Weft 自己的 stage

既然右侧槽进不去，详情就留在 Weft 拥有的槽里：**stage 内部左右分栏**，看板在左、
issue 详情在右。

这仍然满足产品诉求——点看板卡片时人留在看板上，详情从右侧出现——而且不违反任何
一条容器原则：Weft 只在自己拥有的槽里绘制，宿主的 `right` 与 `dock` 完全不碰。

§7 的让位几何是这条路的前提且已落地：宿主一开右侧面板，Weft 的 stage 自动收窄，
Weft 的内部分栏跟着一起窄，不会打架。

代价写明：这与「用宿主原生侧滑承载详情」不是一回事。宿主没有给出可扩展的右侧槽，
在它开放之前，自绘一个属于自己 stage 的抽屉是唯一不靠覆盖的选择。
