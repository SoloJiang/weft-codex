...
test("hosts the issue conversation card outside the clipped sidebar frame", () => {
  const source = buildRendererAgentSource(baseConfig)
  assert.match(source, /weft-codex-issue-panel-root/)
  assert.match(source, /message\.action === "issue-panel\.toggle"/)
  assert.match(source, /message\.action === "issue-panel\.resize"/)
  assert.match(source, /Math\.min\(420, window\.innerHeight - viewportPadding \* 2\)/)
  assert.match(source, /createFrame\("issue-panel"/)
  assert.match(source, /postIssuePanelState\(\)/)
  assert.match(source, /frame !== state\.sidebarFrame/)
  assert.match(source, /frame !== state\.issuePanelFrame/)
})
